...
import { addMessageActionCreator, addMessageActionType } from "../../redux/dialogsPageReducer"
...
const Dialogs = () => {
  const { dialogsData, messageData } = useSelector((state: RootStateOrAny) => state.dialogsPage)
  const { isAuth } = useSelector((state: RootStateOrAny) => state.auth)
  const dispatch = useDispatch()

  const addNewMessageForm = (values: valuesType): addMessageActionType => dispatch(addMessageActionCreator(values.newMessage))
